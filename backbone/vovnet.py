...
from .vovnet_module.constants import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from .vovnet_module.registry import register_model
from .vovnet_module.helpers import build_model_with_cfg
from .vovnet_module.layers import ConvBnAct, SeparableConvBnAct, BatchNormAct2d, ClassifierHead, DropPath,\
...
model_cfgs = dict(
    ...
    hoa_vovnet19b_slim=dict(
        stem_chs=[32, 32, 64],
        stage_conv_chs=[64, 80, 96, 112],
        stage_out_chs=[112, 128, 144, 160],
        layer_per_block=1,
        block_per_stage=[1, 1, 1, 1],
        residual=True,
        depthwise=False,
...
class VovNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, out_stages=[2,3,4], num_classes=1000, global_pool='avg', drop_rate=0., stem_stride=4,
        ...
        for j in range(len(self.out_tuple)):
            stages = []
            for i in range(self.out_tuple[j]):  # num_stages
            ...
            self.blocks.append(nn.Sequential(*stages))
            #self.list_stage = [self.stage[0],self.stage[1],self.stage[2]]
        #self.stages = self.list_stage
    ...
    def forward_features(self, x):
        y = self.stem(x)
        z = []
        for i in range(len(self.out_tuple)):
            yy = self.blocks[i](y)
            z.append(yy)
        return z
...
@register_model
def hoa_vovnet19b_slim(pretrained=False, **kwargs):
    return _create_vovnet('hoa_vovnet19b_slim', pretrained=pretrained, **kwargs)
...
if __name__ == "__main__":
    ...
    print(model)
    test_data = torch.rand(5, 3, 416,416)
